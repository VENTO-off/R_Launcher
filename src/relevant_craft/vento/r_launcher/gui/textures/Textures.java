package relevant_craft.vento.r_launcher.gui.textures;
...
import relevant_craft.vento.r_launcher.manager.texture.LocalTexture;
import relevant_craft.vento.r_launcher.manager.texture.LocalTexture_ListCell;
...
public class Textures implements Initializable {

    public static AnchorPane loadTexturesWindow(ReadOnlyDoubleProperty width, ReadOnlyDoubleProperty height) {
        AnchorPane textures = VENTO.loadGUI(VENTO.GUI.Textures.path);
        textures.setLayoutX(width.getValue() / 2 - textures.getPrefWidth() / 2);
        textures.setLayoutY(height.getValue() / 2 - textures.getPrefHeight() / 2);

        return textures;
    }

    public void onClose_Textures(MouseEvent mouseEvent) {
        VENTO.closeGUI();
    ...
    @FXML
    private JFXListView installedTextures;
    @FXML
    private FontAwesomeIconView texturesFor;
    @FXML
    private JFXTextField search;
    @FXML
    private JFXListView webTextures;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        VENTO.textureManager = new TextureManager(installedTextures, texturesFor, search, webTextures, versionList, categoryList, searchIcon, versionText, categoryText, loading, loadingText, separator, current_cf_project);

        CF_Installer.current_cf_project = CF_Projects.Textures;
        ...
        search.textProperty().addListener((observable, oldValue, newValue) -> TextureManager.findTexturesByString(search.getText()));

        webTextures.setCellFactory((Callback<ListView<CF_Project>, ListCell<CF_Project>>) param -> new CF_ListCell(search));
        webTextures.setOnMouseClicked(event -> webTextures.getSelectionModel().clearSelection());
        webTextures.setFocusTraversable(false);

        installedTextures.setCellFactory((Callback<ListView<LocalTexture>, ListCell<LocalTexture>>) param -> new LocalTexture_ListCell());
        installedTextures.setOnMouseClicked(event -> installedTextures.getSelectionModel().clearSelection());
        installedTextures.setFocusTraversable(false);
    ...
    public void onVersion_Choose(ActionEvent actionEvent) {
        ...
        TextureManager.loadTexturesForVersion(versionList.getValue().getVersionName());
    }

    public void onCategory_Choose(ActionEvent actionEvent) {
        search.setText(null);
        TextureManager.findTexturesByCategory(categoryList.getValue());
    }

    public void onHelp_Textures(ActionEvent actionEvent) {
        DesktopUtils.openUrl("https://r-launcher.su/faq#faq_20");
    }
}
